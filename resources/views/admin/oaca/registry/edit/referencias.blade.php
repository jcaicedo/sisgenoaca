<!--Referencias y Bibliografias-->

<div class="box-body nomostrar" id="references">
  <h4>Referencias y Bibliografías</h4>
  <br>
  <div class="form-group col-md-12">
    <label for="referencias"><span>*</span>Introduzca las referencias y bibliografías</label>
    <textarea name="references_bibliographies" id="references_bibliographies" cols="30" rows="10" class="form-control">{{$content_register->references_bibliographies}}</textarea>
  </div>
</div>
